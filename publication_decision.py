...
def main() -> int:
    ...
    payload = {"NOW_UTC": now.isoformat(), "CURRENT": None if current is None else {"run":current.run,"forecastHour":current.forecast_hour,"validTime":current.valid.isoformat(),"deltaSeconds":int((current.valid-now).total_seconds()),"fresh":is_usable(current.valid,now)}, "CANDIDATE": None if candidate is None else {"run":candidate.run,"forecastHour":candidate.forecast_hour,"validTime":candidate.valid.isoformat(),"deltaSeconds":int((candidate.valid-now).total_seconds()),"fresh":is_usable(candidate.valid,now),"completeTriplet":True}, "FINAL":{"decision":result.decision,"reason":result.reason}}
